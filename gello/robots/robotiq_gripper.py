...
class RobotiqGripper:
    ...
    def _get_var(self, variable: str):
        ...
        :param variable: Name of the variable to retrieve.
        :return: Value of the variable as integer, or None if timeout
        """
        assert self.socket is not None

        # Set socket timeout (in seconds)
        self.socket.settimeout(0.2)  # 200ms timeout

        with self.command_lock:
            try:
                cmd = f"GET {variable}\n"
                self.socket.sendall(cmd.encode(self.ENCODING))
                data = self.socket.recv(1024)  # blocking read (up to 0.2s)
                ...
                print(f"[WARNING] Gripper socket timeout when getting '{variable}'")
                return None

            except Exception as e:
                print(f"[ERROR] Failed to get gripper variable '{variable}': {e}")
                return None
