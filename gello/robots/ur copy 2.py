...
class URRobot(Robot):
    ...
    def __init__(self, robot_ip: str = "192.168.5.101", no_gripper: bool = False):
        ...
        self._script_sock.settimeout(1.0)
        self._script_sock.connect((robot_ip, 30002))  # URScript port           
    ...
    def get_joint_state(self) -> np.ndarray:
        ...
        self._script_sock.sendall(cmd.encode("utf-8"))
        data = self._script_sock.recv(4096)
        text = data.decode("utf-8", errors="ignore")

        import re
        m = re.search(r"\[([\d\.\-+, ]+)\]", text)
        if not m:
            raise RuntimeError(f"관절상태파싱실패:\n{text}")
        robot_joints = np.fromiter(
            (float(x) for x in m.group(1).split(",")), dtype
        )
    ...
    def command_joint_state(self, joint_state: np.ndarray) -> None:
        ...
        self.robot.movej(robot_joints, acc=0.5, vel=0.5)  # ← 변경된 부분
